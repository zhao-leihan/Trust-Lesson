    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({
          ...
          email,
          password,
          role: selectedRole === "mentor" ? "MENTOR" : "LEARNER",
          ...
          {step === 1 ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <button
                type="button"
                onClick={() => setStep(1)}
                className="text-xs font-bold text-indigo-600 hover:underline flex items-center gap-1 mb-1 cursor-pointer"
              >
              ...
              {/* STUDENT-SPECIFIC UNIVERSITY ONBOARDING */}
              {selectedRole === "student" && (
                <div className="bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200 space-y-3.5 mt-2">
                  ...
                  <div>
                    <label className="text-slate-700 font-bold text-[11px] block mb-1.5" htmlFor="university">
                    ...
                    <input
                      ...
                      placeholder="e.g. Stanford University, MIT, University of Indonesia, ITB..."
                      className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-900 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
                    />
                    <p className="text-[10px] text-slate-400 mt-1">
                      Enables verified student credentials, campus cohorts, and alumni mentorship pairings.
                    </p>
                  </div>
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3.5 rounded-xl bg-slate-900 hover:bg-indigo-600 text-white font-bold text-xs shadow-md transition-all flex items-center justify-center gap-2 mt-2 disabled:opacity-60 cursor-pointer"
              >
                {loading
                  ? "Setting Up Account..."
                  : selectedRole === "mentor"

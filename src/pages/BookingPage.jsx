...
export default function BookingPage() {
  ...
  const { courses, addSession, walletAddress, connectWallet } = useAuth();
  ...
  if (isMentor) {
    item = mentors.find((m) => String(m.id) === String(id));
  } else {
    item = (courses || []).find((c) => String(c.id) === String(id));
  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-sm space-y-6">
              ...
              <div className="flex items-start gap-4 pb-6 border-b border-slate-100">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-purple-600 to-indigo-600 text-white font-extrabold text-2xl flex items-center justify-center flex-shrink-0 shadow-md">
                  {isMentor ? item.avatar || item.name[0] : item.mentorAvatar || "G"}
                </div>
